...
plans.json                # [gen] manifest tygodni [{plan_id, from, to, url}] (sort po from)
.github/workflows/
...
plans/
  2026-W30/               # jeden katalog na tydzień (id = tydzień ISO startu)
    ...
    procurement.json      # [wej] rejestr dostawy (pakiety mięsa + wagi) — z raportu „kupiłem…"
...
- **[DECISION] `plan_id` ściśle wg ISO (bez luźnej numeracji).** Id planu = tydzień ISO daty startowej. Pierwotny „W31" dla 24.07 był błędny (to ISO W30) — skorygowane.
- **[DECISION] Podział planów wg granic tygodni ISO.** Plan wydłużony przekraczający granicę tygodnia dzielimy na osobne plany: **`2026-W30`** (23–26.07, część od czwartku) i **`2026-W31`** (27.07–02.08, pełny pn–nd). Root wybiera właściwy tydzień client-side (patrz decyzja o resolverze). Rewizja W31 podbita do 2 (kontrakt skonsumowany → zmiana = nowa rewizja).
- **[DECISION] Root wybiera tydzień client-side z manifestu.** `build.py` generuje `plans.json` (`[{plan_id, from, to, url}]` z `period` każdego `demand.json`, sort po `from`) — artefakt zależny **wyłącznie od zawartości `plans/`**, bez pola „dziś" (build deterministyczny). Root `index.html` to resolver JS: reguła **pokrywający dziś → najbliższy przyszły → najnowszy**, data z urządzenia (lokalna strefa rodziny), `location.replace`. Bez JS / przy błędzie fetch — widoczna lista linków do planów (fallback). Skutek: 28.07 → W31 (pokrywający), dzień po ostatnim planie → najnowszy; **23.07 → W30**, bo W30 pokrywa 23–26 (dzień łososia) — reguła „pokrywający najpierw" świadomie pokazuje bieżący tydzień.
...
- **[DECISION] Przedziały mięsa per danie** (migracja 02). Profil w `preferences.md` (`meat_profile`): **dorosły 170–220 g — ZATWIERDZONE (Marcin, PR#2); nadpisuje 180–220 z [DECISION] migracji 02** (przedział nadal WYLICZANY z profilu, bez stałej w logice); mnożnik 0,3–0,4× per dziecko (definiowalny — dzieci rosną). Przedział dania liczony ze składu jedzących: `[adult_min × (adults + Σ min), adult_max × (adults + Σ max)]` = obecnie **272–396 g** (~290–400 g). Wektory testowe w `build.py` (brzegi 270/275/280) blokują dryf profilu. Zastępuje sztywne nominały per mięso; mięso w kartach = `unit: danie`, łosoś na wagę.
- **[DECISION] Pętla zaopatrzeniowa — trzy zapisy per plan.** `demand.json` (zamówienie), `procurement.json` (dostawa), `journal.json` (wykonanie). Schema procurement: `{ plan_id, items: [{ ingredient, packages: [{ weight_g, use_by? }], note? }] }` (nazwy kanoniczne, klucze EN, bez timestampów → determinizm). Tryb renderu **wyprowadzany z danych PER MIĘSO**: brak wpisu → orientacyjny (przedział łączny na tydzień), wpis → konkretny (`classify_package` na wagach; przypisanie pakiety→obiady: `use_by` rosnąco ↔ data obiadu rosnąco, potem większy pakiet → wcześniejszy obiad; ostrzeżenia po ludzku „może być mało mięsa" / „wyjdzie więcej — nadmiar do lodówki"). Raport zakupów **nie zmienia `revision`**; przemodelowanie menu = rewizja jak dotąd. Pakiet poza przedziałem → propozycja rewizji w podsumowaniu, nie samowolna zmiana. Bramki `--check`: składniki procurement kanoniczne; golden testy linii mięsnych w obu trybach.
- **[DECISION] Strona mówi językiem kuchni, nie modelu (zasada trwała).** Lista słów zakazanych na generowanych stronach w jednym miejscu w `build.py` (`FORBIDDEN_ON_PAGE`) + bramka `scan_forbidden` w `--check` (całe słowa, bez wielkości liter — „śniadanie"/„wydanie" bezpieczne). Zakazane: `danie`, `przedział`, `anchor`, `wielodaniowe`, `qty_range`, `demand`, `procurement`, `journal`, `revision`, `klasyfikacja`, nazwy trybów. Mapa: danie→obiad, dostawa→„kupione", przedział→„~X–Y g". Notki `advance_prep`/mięsne z kart są wewnętrzne — na stronę tylko kuchenne tłumaczenie. Obowiązuje w obu trybach i niezależnie od treści pojedynczych zleceń (reguła też w `CLAUDE.md`). Test Moniki: lista zakupów + karta dnia czytelne bez słowa wyjaśnienia.
- **[DECISION] Niedziela 2026-W30: restauracja → obiad** (rev 7). Nadwyżka indyka (drugie danie z opakowania 625 g, „spożyć do" 27.07) zużyta świeżo (zamrażarka w naprawie) jako nowa karta `kotlety-z-piersi-panierowane` (technika schabowego) + mizeria i ziemniaki. `procurement.json` W30 wpisuje to opakowanie → karta niedzieli i pulpeciki startują w trybie konkretnym („połowa opakowania 625 g").
...
- **[DECISION] Batch mielone = 2 dania zakupowo** (≈ 580–790 g łącznie); uchyla podział pojedynczej tacki pół na pół. **Sos rozprzężony** — osobne, opcjonalne danie (składnik-bazowy). Reguła „oba dania tego samego dnia" jest **warunkowa** — obowiązuje przy niedostępnej zamrażarce.
- **[DECISION] Gospodarstwo ma zamrażarkę (trwale)** — uchyla „brak zamrażarki" z kickoffu (EXTRACT §6, do rewizji). **Status bieżący w `preferences.md`; obecnie W NAPRAWIE → mrożenie niedostępne do odwołania.** Póki niedostępna: nie planujemy mrożenia, nadwyżki → lodówka + `leftovers` w journalu / slot „z lodówki", mielone na 2 dania tego samego dnia. Flip po naprawie = zmiana jednej linii statusu w `preferences.md`, bez migracji. `build.py`/karty czytają zasadę; planując, sprawdzaj status.
...
- **[DECISION] Strategia przy psuciu = „zrób więcej i zamroź" — tylko gdy zamrażarka dostępna.** Gdy niedostępna (obecnie W NAPRAWIE): świeże mięso zużyj świeżo/z lodówki, nadwyżkę zjedz jako `leftovers` (journal), a nie wpychaj mrożenia. Gdy dostępna: większy batch dania mrożalnego (sos, pulpeciki, kotlety) zamiast wyrzucać.
