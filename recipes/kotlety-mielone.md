---
id: kotlety-mielone
name: Kotlety mielone
type: obiad
tags: [wieprzowina, smażone, klasyka]
base_servings: 2
advance_prep: "tego samego dnia ugotuj sos-pomidorowy-z-mielonym — jedna tacka łopatki (400 g) na dwa dania"
components: []
ingredients:
  - { name: "łopatka mielona wieprzowa", qty: 200, unit: g, note: "½ tacki 400 g" }
  - { name: "jajka", qty: 1, unit: szt }
  - { name: "bułka tarta", qty: 3, unit: łyżka, note: "+ do obtaczania" }
  - { name: "cebula", qty: 0.25, unit: szt, note: "drobno posiekana" }
  - { name: "czosnek", qty: 0.5, unit: ząbek, note: "przeciśnięty" }
  - { name: "kasza gryczana", qty: 100, unit: g, note: "1 torebka" }
  - { name: "brokuł", qty: 0.33, unit: szt, note: "garść różyczek" }
  - { name: "marchew", qty: 1, unit: szt }
  - { name: "groszek zielony konserwowy", qty: 4, unit: łyżka }
pantry: [olej, masło, sól, pieprz czarny mielony]
---

# Kotlety mielone

Dodatki: kasza gryczana · brokuł · marchewka z groszkiem

> Tego samego dnia ugotuj też [sos pomidorowy](sos-pomidorowy-z-mielonym.md) — jedna tacka łopatki (400 g) na dwa dania.

## Przygotowanie

1. Wymieszać: mielone + jajko + 3 łyżki bułki tartej + cebula + czosnek. Dobrze wyrobić ręką.
2. Dodać sól i pieprz. Uformować ~5 kotletów, obsypać bułką tartą.
3. Smażyć na oleju 4–5 min z każdej strony.
4. Kaszę gryczaną ugotować wg opisu na opakowaniu.
5. Brokuł: różyczki gotować 7–8 min, podać z masłem.
6. Marchew pokroić w plasterki, ugotować 10 min. Odcedzić, dodać groszek i masło.

## Historia i feedback

| Data | Plan | Ocena | Notatki |
|---|---|---|---|
| 2026-07-27 | 2026-W31 | — | pierwsze użycie w bazie |
